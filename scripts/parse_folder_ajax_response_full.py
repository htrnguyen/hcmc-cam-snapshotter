...
OUTPUT_JSON_FULL = "../camera_catalog/camera_catalog_full.json"
OUTPUT_JSON_LIGHT = "../camera_catalog/camera_catalog_light.json"
...
MEDIA_PATTERNS = re.compile(
    r"(snapshot|hls|m3u8|rtsp|stream|video|mp4|jpeg|jpg|gif)",
    re.IGNORECASE
)
...
# Value/DefaultValue có thể là string/bool/null/number
KVP_VALUE_RE = re.compile(r'"Value"\s*:\s*(?P<val>"[^"]*"|true|false|null|-?\d+(?:\.\d+)?)', re.IGNORECASE)
KVP_DEFAULT_RE = re.compile(r'"DefaultValue"\s*:\s*(?P<val>"[^"]*"|true|false|null|-?\d+(?:\.\d+)?)', re.IGNORECASE)
...
def choose_loc_raw(props_norm: Dict[str, Any], node_title: Optional[str], code: Optional[str]) -> Optional[str]:
    return props_norm.get("cam_location") or node_title or code

def build_expand_link(cam_id: Optional[str], loc_raw: Optional[str]) -> Dict[str, Optional[str]]:
    if not cam_id or not loc_raw:
        return {"loc_raw": loc_raw, "loc_encoded": None, "url": None}
    # Luôn encode từ chuỗi thô (không dựa vào input đã encode)
    loc_encoded = urllib.parse.quote(str(loc_raw), safe="")
    url = (
        "https://giaothong.hochiminhcity.gov.vn/expandcameraplayer/"
        f"?camId={cam_id}&camLocation={loc_encoded}&camMode=camera"
    )
    return {"loc_raw": loc_raw, "loc_encoded": loc_encoded, "url": url}
...
# ====== Build properties_raw từ list Properties (giữ NGUYÊN thông tin) ======
def build_properties_raw_from_list(prop_list: Any) -> Dict[str, Dict[str, Any]]:
    ...
    for p in prop_list:
        ...
        if not name:
            # Nếu không có Name, tạo key ẩn nhưng vẫn giữ để không mất dữ liệu
            name = f"__NO_NAME__:{hashlib.sha1(_json_dumps_canonical(p).encode('utf-8')).hexdigest()[:10]}"
        # Copy toàn bộ fields ngoại trừ Name (Name sẽ là key)
...
def normalize_properties(properties_raw: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    norm: Dict[str, Any] = {}
    # đi qua từng Name gốc
    for raw_key, entry in (properties_raw or {}).items():
        # Giá trị chính: ưu tiên Value -> DefaultValue -> None
        val = None
        ...
        if friendly:
            ...
            # không map thì bỏ qua ở properties_norm (vì đã giữ nguyên ở properties_raw)
            pass

    # Hợp nhất district từ cả District/Disctrict nếu cả hai có
    if "district" not in norm:
        # thử đọc trực tiếp nếu có key không map
        for raw_key, entry in (properties_raw or {}).items():
    ...
    # Nếu chưa có code mà Title có trong raw -> sẽ dùng ở tầng ngoài
    return norm
...
def detect_media_flags(properties_raw: Dict[str, Dict[str, Any]]) -> Dict[str, bool]:
    ...
    for k, entry in (properties_raw or {}).items():
        ...
        if isinstance(entry, dict):
            val = entry.get("Value")
        # Kiểm tra cả key lẫn value (nếu là chuỗi)
        val_hit = False
        ...
        if key_hit or val_hit:
            ...
            # phân biệt chi tiết
            if re.search(r"snapshot", k, re.IGNORECASE) or (isinstance(val, str) and "snapshot" in val.lower()):
                flags["has_snapshot_url"] = True
            if re.search(r"hls|m3u8", k, re.IGNORECASE) or (isinstance(val, str) and re.search(r"hls|m3u8", val, re.IGNORECASE)):
                flags["has_hls"] = True
            if re.search(r"rtsp", k, re.IGNORECASE) or (isinstance(val, str) and "rtsp" in val.lower()):
                flags["has_rtsp"] = True
...
def parse_json_path(data: Any) -> List[Dict[str, Any]]:
    ...
    for node in walk(data):
        ...
        expand_link = build_expand_link(cam_id, loc_raw)

        # media_flags
        media_flags = detect_media_flags(properties_raw)

        # possible_duplicate_group: ưu tiên cam_id, nếu không có thì băm properties_raw + Path
        group_key_src = {}
        if cam_id:
...
def parse_regex_fallback(text: str) -> List[Dict[str, Any]]:
    ...
    for m in NODE_ANCHOR_RE.finditer(text):
        # Lấy blob quanh vị trí node "CAMERA"
        ...
        if type_m: node_meta["Type"] = type_m.group(1)
        if title_m: node_meta["Title"] = title_m.group(1)
        if name_m: node_meta["Name"] = name_m.group(1)
        if path_m: node_meta["Path"] = path_m.group(1)
        if created_m: node_meta["Created"] = created_m.group(1)
        if modified_m: node_meta["Modified"] = modified_m.group(1)
        ...
        node_meta["raw_node_hash"] = sha256_of_obj(node_meta)  # fallback: băm meta

        # Tìm block Properties
        props_block_m = PROPS_BLOCK_RE.search(blob)
        properties_raw: Dict[str, Dict[str, Any]] = {}
        if props_block_m:
            props_block = props_block_m.group(1)

            # Cắt từng object { ... } trong mảng Properties
            for obj_m in PROP_OBJ_RE.finditer(props_block):
                ...
                # lưu cả block thô phòng hờ (không bắt buộc, nhưng hữu ích)
...
def parse_json_like_scalar(s: str) -> Any:
    ...
    try:
        ...
        return s  # fallback: trả nguyên văn
...
def main():
    ...
    # KHÔNG dedupe: giữ nguyên thứ tự (nhưng có thể sort nhẹ để ổn định)
    # Sắp theo Title rồi CamId (nếu có)
    def sort_key(rec: Dict[str, Any]):
        node_meta = rec.get("node_meta") or {}
        title = node_meta.get("Title") or rec.get("properties_norm", {}).get("code") or ""
    ...
    for rec in items:
        ...
        light_rows.append({
            ...
            "expand_url": (rec.get("expand_link") or {}).get("url"),
            "expand_loc_raw": (rec.get("expand_link") or {}).get("loc_raw"),
            "has_snapshot_url": (rec.get("media_flags") or {}).get("has_snapshot_url"),
            "has_hls": (rec.get("media_flags") or {}).get("has_hls"),
            "has_rtsp": (rec.get("media_flags") or {}).get("has_rtsp"),
            "possible_duplicate_group": rec.get("possible_duplicate_group"),
            # Tham chiếu đến record đầy đủ để tiện tra cứu
            "_full_ref_hash": sha256_of_obj(rec),
        })

    with open(OUTPUT_JSON_LIGHT, "w", encoding="utf-8") as f:
        json.dump(light_rows, f, ensure_ascii=False, indent=2)

    # Thống kê nhanh
    total = len(items)
    with_snapshot = sum(1 for r in items if (r.get("media_flags") or {}).get("has_snapshot_url"))
    ...
    print(f"[OK] Full JSON          : {OUTPUT_JSON_FULL}")
    print(f"[OK] Light JSON         : {OUTPUT_JSON_LIGHT}")
    print(f"[Info] Media flags      : snapshot={with_snapshot}, hls={with_hls}, rtsp={with_rtsp}")
